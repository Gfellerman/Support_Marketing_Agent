...
// Workflow step types matching the visual builder
export interface WorkflowStep {
  id: string;
  type: "trigger" | "send_email" | "delay" | "condition" | "add_tag" | "remove_tag" | "webhook" | "update_field";
 ...
 */
function validateTriggerStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function validateDelayStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function validateEmailStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function validateConditionStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function validateTagStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function validateWebhookStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function validateUpdateFieldStep(step: WorkflowStep): ValidationError[] {
  const errors: ValidationError[] = [];
 ...
 */
function findDisconnectedSteps(steps: WorkflowStep[]): string[] {
  ...
  while (queue.length > 0) {
    ...
    if (currentStep.type === "condition") {
      const trueBranch = currentStep.config?.trueBranch as string | undefined;
      const falseBranch = currentStep.config?.falseBranch as string | undefined;
 ...
 */
function findUnreachableSteps(steps: WorkflowStep[]): string[] {
