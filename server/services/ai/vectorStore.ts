...
class TFIDFVectorStore {
  ...
  private documentToVector(text: string): number[] {
  ...
  async initialize(organizationId: number, forceRefresh = false): Promise<void> {
  ...
  async search(query: string, topK = 3, minScore = 0.1): Promise<SearchResult[]> {
...
// Singleton instance
let vectorStore: TFIDFVectorStore | null = null;

export function getVectorStore(): TFIDFVectorStore {
  if (!vectorStore) {
...
export async function refreshKnowledgeIndex(organizationId: number): Promise<void> {
  const store = getVectorStore();
