...
import type { TicketCategory, PriorityLevel } from "./classification";
 ...
 */
export function buildResponseSystemPrompt(context: ResponseGenerationContext): string {
...
- For legal threats, acknowledge and offer to escalate to a supervisor`;
  ...
  if (context.customer) {
    ...
    prompt += `\n\nCUSTOMER CONTEXT:`;
    if (c.name) prompt += `\n- Name: ${c.name}`;
    if (c.totalOrders !== undefined) prompt += `\n- Total Orders: ${c.totalOrders}`;
    if (c.lifetimeValue !== undefined) prompt += `\n- Lifetime Value: $${c.lifetimeValue.toFixed(2)}`;
...
export const RESPONSE_TEMPLATES = {
  orderStatus: (orderNumber: string, status: string, tracking?: string) => 
    `I can see your order #${orderNumber} is currently ${status}.${tracking ? ` You can track it using: ${tracking}` : ""} Is there anything else I can help you with?`,
  
  refundInitiated: (orderNumber: string, amount: string, timeframe: string) =>
    `I've initiated a refund of ${amount} for order #${orderNumber}. You should see this reflected in your account within ${timeframe}. Is there anything else I can help you with?`,
  
  escalation: () =>
    `I understand this is an important matter. I'm going to escalate this to a senior team member who can better assist you. They'll be in touch within 24 hours. Is there anything else I can help with in the meantime?`,
  
  needsMoreInfo: (infoNeeded: string) =>
    `To better assist you, could you please provide ${infoNeeded}? This will help me resolve your issue more quickly.`,
};
