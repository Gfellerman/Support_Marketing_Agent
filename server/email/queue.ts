...
import { sendEmail, EmailRecipient, EmailContent } from './sendgrid';
 ...
 */
const emailWorker = queueEnabled && redisConnection ? new Worker<SendEmailJobData>(
  'email-sending',
 ...
 */
export async function queueEmail(data: SendEmailJobData): Promise<Job<SendEmailJobData> | null> {
  if (!emailQueue) {
 ...
 */
export async function queueCampaign(data: SendCampaignJobData): Promise<(Job<SendEmailJobData> | null)[]> {
  if (!emailQueue) {
    console.warn('[EmailQueue] Queue not available, campaign sending disabled');
    return [];
  }
  
  const jobs: Promise<Job<SendEmailJobData>>[] = [];
