...
import { buildResponseSystemPrompt, buildResponseUserPrompt } from "./prompts/response";
import { buildResponseContext } from "./contextBuilder";
import { ORDER_RESPONSE_TEMPLATES, hydrateTemplate, getQuickActionsForIssue } from "./prompts/orderResponses";
...
function extractSuggestedActions(content, issueType) {
    ...
    if (lowerContent.includes('replacement'))
        actions.push('send_replacement');
    if (lowerContent.includes('return label'))
        actions.push('generate_return_label');
    if (lowerContent.includes('tracking'))
        actions.push('share_tracking_link');
    if (lowerContent.includes('discount') || lowerContent.includes('coupon'))
        actions.push('offer_discount');
    if (lowerContent.includes('escalate') || lowerContent.includes('supervisor'))
...
export async function generateResponse(input) {
    ...
    if (input.useKnowledgeBase !== false) {
        try {
            ...
            knowledgeArticles = await searchKnowledge(orgId, searchQuery, input.maxKnowledgeArticles || 3, 0.15 // minimum relevance threshold
            );
    ...
    const userPrompt = buildResponseUserPrompt(input.ticketSubject, input.ticketContent, input.additionalContext ? [{ role: 'agent', content: input.additionalContext }] : []);
    // Generate response
 ...
 */
export async function generateMultipleResponses(input) {
    const tones = ['professional', 'friendly', 'empathetic'];
    // Generate all three in parallel
    const responsePromises = tones.map(tone => generateResponse({ ...input, tone }));
 ...
 */
export function generateTemplateResponse(issueType, tone, order, customerName) {
