 ...
 */
export async function getDashboardMetrics(organizationId, dateRange) {
    ...
    const interactionStats = await db
        .select({
        total: count(),
        used: sql `SUM(CASE WHEN ${aiInteractions.wasUsed} = true THEN 1 ELSE 0 END)`,
    ...
    const feedbackStats = await db
        .select({
        total: count(),
        positive: sql `SUM(CASE WHEN ${aiFeedback.rating} = 'positive' THEN 1 ELSE 0 END)`,
        negative: sql `SUM(CASE WHEN ${aiFeedback.rating} = 'negative' THEN 1 ELSE 0 END)`,
        edited: sql `SUM(CASE WHEN ${aiFeedback.wasEdited} = true THEN 1 ELSE 0 END)`,
 ...
 */
export async function getMetricsByCategory(organizationId, dateRange) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    ...
    const results = await db
        ...
        total: count(),
        used: sql `SUM(CASE WHEN ${aiFeedback.wasUsed} = true THEN 1 ELSE 0 END)`,
        positive: sql `SUM(CASE WHEN ${aiFeedback.rating} = 'positive' THEN 1 ELSE 0 END)`,
    })
        .from(aiFeedback)
        .where(and(...conditions))
 ...
 */
export async function getTrends(organizationId, dateRange, granularity = "day") {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    ...
    const results = await db
        .select({
        date: sql `DATE_FORMAT(${aiInteractions.createdAt}, ${dateFormat})`,
        total: count(),
        used: sql `SUM(CASE WHEN ${aiInteractions.wasUsed} = true THEN 1 ELSE 0 END)`,
        avgConfidence: avg(aiInteractions.confidenceScore),
    })
        .from(aiInteractions)
        .where(and(eq(aiInteractions.organizationId, organizationId), gte(aiInteractions.createdAt, dateRange.startDate), lte(aiInteractions.createdAt, dateRange.endDate)))
    ...
    const feedbackTrends = await db
        .select({
        date: sql `DATE_FORMAT(${aiFeedback.createdAt}, ${dateFormat})`,
        positive: sql `SUM(CASE WHEN ${aiFeedback.rating} = 'positive' THEN 1 ELSE 0 END)`,
        total: count(),
    })
        .from(aiFeedback)
        .where(and(eq(aiFeedback.organizationId, organizationId), gte(aiFeedback.createdAt, dateRange.startDate), lte(aiFeedback.createdAt, dateRange.endDate)))
 ...
 */
export async function getAgentAdoption(organizationId, dateRange) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
 ...
 */
export async function getTopPerformingTemplates(organizationId, limit = 10) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    const results = await db
 ...
 */
export async function getKnowledgeBaseMetrics(organizationId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
