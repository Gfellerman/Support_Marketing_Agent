 ...
 */
export async function buildCustomerContext(customerId, organizationId) {
    try {
        ...
        const customerResults = await db.select().from(contacts)
            .where(and(eq(contacts.id, parseInt(customerId)), eq(contacts.organizationId, parseInt(organizationId))))
            .limit(1);
        ...
        const customerOrders = await db.select().from(orders)
            .where(and(eq(orders.contactId, parseInt(customerId)), eq(orders.organizationId, parseInt(organizationId))))
            .orderBy(desc(orders.orderedAt))
            .limit(10);
        // Fetch tickets (last 5)
        const customerTickets = await db.select().from(tickets)
            .where(and(eq(tickets.contactId, parseInt(customerId)), eq(tickets.organizationId, parseInt(organizationId))))
        ...
        const ticketSummaries = customerTickets.map(t => `[${t.status}] ${t.subject}: ${t.aiCategory || 'uncategorized'} - ${t.aiSentiment || 'unknown'} sentiment`);
        return {
            customer: {
 ...
 */
export async function buildResponseContext(ticketId, organizationId) {
    try {
        const db = await getDb();
        if (!db)
            return {};
        ...
        const ticketResults = await db.select().from(tickets)
            .where(and(eq(tickets.id, parseInt(ticketId)), eq(tickets.organizationId, parseInt(organizationId))))
            .limit(1);
 ...
 */
export async function buildOrderContext(orderNumber, organizationId) {
    try {
        const db = await getDb();
        if (!db)
            return null;
        const orderResults = await db.select().from(orders)
            .where(and(eq(orders.orderNumber, orderNumber), eq(orders.organizationId, parseInt(organizationId))))
            .limit(1);
