...
import { CLASSIFICATION_SYSTEM_PROMPT, buildClassificationPrompt, TICKET_CATEGORIES, PRIORITY_LEVELS, } from "./prompts/classification";
import { SENTIMENT_SYSTEM_PROMPT, buildSentimentPrompt, SENTIMENT_LABELS, } from "./prompts/sentiment";
/**
 ...
 */
export async function classifyTicket(subject, message, customerContext) {
    ...
    try {
        ...
        const [classificationResult, sentimentResult] = await Promise.all([
            fastClassify(CLASSIFICATION_SYSTEM_PROMPT, buildClassificationPrompt(subject, message, customerContext)),
            fastClassify(SENTIMENT_SYSTEM_PROMPT, buildSentimentPrompt(message)),
        ]);
        // Parse responses
        ...
        const adjustedPriority = adjustPriority(classification.priority, sentiment.label, customerContext);
        // Calculate combined confidence
        const combinedConfidence = (classification.confidence + sentiment.confidence) / 2;
        const processingTimeMs = Date.now() - startTime;
        console.log(`[TicketClassifier] Classified ticket in ${processingTimeMs}ms:`, `category=${classification.category}, priority=${adjustedPriority}, sentiment=${sentiment.label}`);
        return {
...
export async function analyzeSentiment(message) {
    try {
        const result = await fastClassify(SENTIMENT_SYSTEM_PROMPT, buildSentimentPrompt(message));
