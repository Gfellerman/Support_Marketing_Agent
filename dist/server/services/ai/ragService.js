...
import { buildCustomerContext } from "./contextBuilder";
import { getDb } from "../../db";
import { orders } from "../../../drizzle/schema";
import { eq, and } from "drizzle-orm";
// Build RAG context from knowledge base
export async function buildRAGContext(organizationId, query, maxArticles = 3, minRelevance = 0.15) {
    try {
...
function formatKnowledgeContext(ragContext) {
...
export async function generateRAGResponse(input) {
    const startTime = Date.now();
    // 1. Build RAG context from knowledge base
    const searchQuery = `${input.ticketSubject} ${input.ticketContent}`;
    const ragContext = await buildRAGContext(input.organizationId, searchQuery, input.maxKnowledgeArticles || 3);
    ...
    if (input.orderNumber) {
        ...
        if (db) {
            const orderResults = await db
                .select()
                .from(orders)
                .where(and(eq(orders.organizationId, input.organizationId), eq(orders.orderNumber, input.orderNumber)))
                .limit(1);
...
export async function generateMultipleRAGResponses(input) {
...
export async function hasRelevantKnowledge(organizationId, query, minRelevance = 0.2) {
