...
export const contactsRouter = router({
    ...
    list: protectedProcedure
        ...
        if (input.search) {
            conditions.push(or(like(contacts.email, `%${input.search}%`), like(contacts.firstName, `%${input.search}%`), like(contacts.lastName, `%${input.search}%`)));
        }
    ...
    get: protectedProcedure
        ...
        const contact = await db
            .select()
            .from(contacts)
            .where(and(eq(contacts.id, input.id), eq(contacts.organizationId, organizationId)))
            .limit(1);
        if (contact.length === 0) {
    ...
    create: protectedProcedure
        ...
        const existing = await db
            .select()
            .from(contacts)
            .where(and(eq(contacts.email, input.email), eq(contacts.organizationId, organizationId)))
            .limit(1);
