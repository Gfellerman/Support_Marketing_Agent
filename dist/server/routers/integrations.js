...
export const integrationsRouter = router({
    ...
    shopifyCallback: protectedProcedure
        ...
        const tokenData = await ShopifyClient.exchangeCodeForToken(input.shop, input.code, config);
        // Get shop info
        ...
        for (const topic of Object.values(SHOPIFY_WEBHOOK_TOPICS)) {
            try {
                await client.createWebhook(topic, `${process.env.APP_URL || 'http://localhost:3000'}/api/webhooks/shopify`);
            }
            catch (error) {
    ...
    woocommerceConnect: protectedProcedure
        ...
        for (const topic of Object.values(WOOCOMMERCE_WEBHOOK_TOPICS)) {
            try {
                await client.createWebhook(topic, `${process.env.APP_URL || 'http://localhost:3000'}/api/webhooks/woocommerce`);
            }
            catch (error) {
                console.error(`Failed to create webhook for ${topic}:`, error);
            }
    ...
    sync: protectedProcedure
        ...
        const [integration] = await db
            .select()
            .from(integrations)
            .where(and(eq(integrations.id, input.integrationId), eq(integrations.organizationId, organizationId)))
            .limit(1);
        if (!integration) {
        ...
        if (integration.type === 'shopify') {
            ...
            result = await syncEngine.fullShopifySync(credentials.accessToken, credentials.shop);
        }
        else if (integration.type === 'woocommerce') {
            const credentials = integration.credentials;
            result = await syncEngine.fullWooCommerceSync(credentials.storeUrl, credentials.consumerKey, credentials.consumerSecret);
        }
        else {
