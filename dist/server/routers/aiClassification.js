...
import { classifyTicket, analyzeSentiment, } from "../services/ai/ticketClassifier";
 ...
 */
async function getOrganizationId(db, userId) {
...
export const aiClassificationRouter = router({
    ...
    classifyById: protectedProcedure
        ...
        const classification = await classifyTicket(ticket.subject, ticket.subject, // Use subject as message if no message content
        customerContext);
        // Log the interaction
        await logAIInteraction(db, organizationId, input.ticketId, "classification", "llama-4-scout", classification.processingTimeMs, classification.confidence, ticket.subject, JSON.stringify({
    ...
    classifyContent: protectedProcedure
        ...
        if (!db)
            throw new Error("Database not available");
        const organizationId = await getOrganizationId(db, ctx.user.id);
        const classification = await classifyTicket(input.subject, input.message, input.customerContext);
        // Log the interaction
        await logAIInteraction(db, organizationId, null, "classification", "llama-4-scout", classification.processingTimeMs, classification.confidence, `${input.subject}: ${input.message}`.substring(0, 200));
        return classification;
    }),
    /**
    ...
    analyzeSentiment: protectedProcedure
        ...
        // Log the interaction
        await logAIInteraction(db, organizationId, null, "sentiment", "llama-4-scout", Date.now() - startTime, sentiment.confidence, input.message.substring(0, 200), JSON.stringify(sentiment));
        return sentiment;
    }),
    /**
    ...
    getAnalytics: protectedProcedure
        ...
        const recentInteractions = interactions.filter((i) => new Date(i.createdAt) >= cutoffDate);
        const analytics = {
            ...
            avgLatencyMs: recentInteractions.length > 0
                ? Math.round(recentInteractions.reduce((sum, i) => sum + (i.latencyMs || 0), 0) /
                    recentInteractions.length)
                : 0,
            avgConfidence: recentInteractions.length > 0
                ? recentInteractions.reduce((sum, i) => sum + (i.confidenceScore ? parseFloat(i.confidenceScore) : 0), 0) / recentInteractions.length
                : 0,
            usageRate: recentInteractions.length > 0
