...
import { workflowTemplates as builtInTemplates, getTemplatesByCategory, getTemplateById as getBuiltInTemplateById, searchTemplatesByTags, getAllCategories, getAllTags, } from "../workflows/templates";
...
export const templatesRouter = router({
        ...
        if (input.includeUserSaved) {
            ...
            if (orgs.length > 0) {
                ...
                let query = db.select().from(workflowTemplatesTable)
                    .where(or(eq(workflowTemplatesTable.organizationId, orgId), eq(workflowTemplatesTable.isPublic, true)))
    ...
    save: protectedProcedure
        ...
        category: z.enum(['ecommerce', 'saas', 'retail', 'services', 'general', 'custom']),
        tags: z.array(z.string()).optional(),
    ...
    delete: protectedProcedure
        ...
        await db.delete(workflowTemplatesTable)
            .where(and(eq(workflowTemplatesTable.id, input.id), eq(workflowTemplatesTable.organizationId, orgId)));
        return { success: true };
    }),
    /**
     * Get all categories
