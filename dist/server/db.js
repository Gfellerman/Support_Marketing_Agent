...
import { users, organizations, contacts, emailCampaigns, workflows, tickets, orders, integrations, analyticsEvents, } from "../drizzle/schema";
import { ENV } from './_core/env';
// Use specific type instead of ReturnType<typeof drizzle> to avoid overload mismatches
let _db = null;
...
// Contact queries
export async function getContactsByOrganization(organizationId, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
...
export async function getCampaignsByOrganization(organizationId, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
...
// Workflow queries
export async function getWorkflowsByOrganization(organizationId, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
...
// Ticket queries
export async function getTicketsByOrganization(organizationId, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
...
// Order queries
export async function getOrdersByOrganization(organizationId, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
...
// Integration queries
export async function getIntegrationsByOrganization(organizationId) {
    const db = await getDb();
    if (!db)
        return [];
...
// Analytics queries
export async function getAnalyticsEventsByOrganization(organizationId, eventType, limit = 100) {
    const db = await getDb();
    if (!db)
        return [];
...
export async function getDashboardStats(organizationId) {
    ...
    const [contactsCount] = await db.select({ count: sql `count(*)` })
        .from(contacts)
        .where(eq(contacts.organizationId, organizationId));
    const [campaignsCount] = await db.select({ count: sql `count(*)` })
        .from(emailCampaigns)
        .where(eq(emailCampaigns.organizationId, organizationId));
