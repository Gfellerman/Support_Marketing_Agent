...
export async function enrollContact(params) {
    ...
    const existing = await db
        ...
        .where(and(eq(workflowEnrollments.workflowId, workflowId), eq(workflowEnrollments.contactId, contactId), eq(workflowEnrollments.status, 'active')))
        .limit(1);
...
export async function triggerWorkflows(params) {
    ...
    const matchingWorkflows = await db
        .select()
        .from(workflows)
        .where(and(eq(workflows.triggerType, trigger), eq(workflows.status, 'active')));
 ...
 */
export async function getWorkflowAnalytics(workflowId) {
    const db = await getDb();
    if (!db)
        throw new Error('Database not available');
