...
const workflowWorker = schedulerEnabled && redisConnection ? new Worker('workflow-steps', async (job) => {
 ...
 */
export async function scheduleWorkflowStep(data, delayMs) {
    if (!workflowQueue) {
 ...
 */
export async function scheduleImmediateStep(data) {
    if (!workflowQueue) {
        // Execute immediately
        await executeWorkflowStep(data);
        return null;
 ...
 */
export async function cancelScheduledStep(jobId) {
    if (!workflowQueue)
        return;
 ...
 * Clean old jobs
 */
export async function cleanScheduler() {
    if (!workflowQueue)
        return;
