...
function mapShopifyOrder(order, organizationId, contactId) {
    return {
        ...
        source: 'shopify',
        status: order.financialStatus, // Cast to enum
...
function mapWooCommerceOrder(order, organizationId, contactId) {
    return {
        ...
        source: 'woocommerce',
        status: order.status, // Cast to enum
        total: order.total,
        currency: order.currency,
        items: order.lineItems.map(item => ({
            product_id: item.productId.toString(),
                ...
                for (const customer of customers) {
                    try {
                        ...
                        const existing = await db
                            .select()
                            .from(contacts)
                            .where(and(eq(contacts.organizationId, this.organizationId), eq(contacts.email, customer.email)))
                            .limit(1);
                ...
                for (const order of shopifyOrders) {
                    try {
                        ...
                        let contact = await db
                            .select()
                            .from(contacts)
                            .where(and(eq(contacts.organizationId, this.organizationId), eq(contacts.email, order.email)))
                            .limit(1);
                        let contactId;
                        if (contact.length === 0) {
                        ...
                        const existing = await db
                            .select()
                            .from(orders)
                            .where(and(eq(orders.organizationId, this.organizationId), eq(orders.externalId, order.id.toString())))
                            .limit(1);
                        if (existing.length > 0) {
                ...
                for (const order of wooOrders) {
                    try {
                        ...
                        let contact = await db
                            .select()
                            .from(contacts)
                            .where(and(eq(contacts.organizationId, this.organizationId), eq(contacts.email, order.billing.email)))
                            .limit(1);
                        let contactId;
                        if (contact.length === 0) {
                            // Create contact from order data
